...
void ZmqMiddleware::start()
{
    ...
    switch (state_)
    {
        ...
        case Stopped:
        {
            ...
            // N.B. We absolutely MUST have AT LEAST 2 invoke threads
            // as rebinding is invoked within 2-way invocations.
            {
                lock_guard<mutex> lock(data_mutex_);
                invokers_.reset(new ThreadPool(2));
            }
            state_ = Started;
...
MWQueryCtrlProxy ZmqMiddleware::add_query_ctrl_object(QueryCtrlObjectBase::SPtr const& ctrl)
{
    ...
    try
    {
        ...
        return ZmqQueryCtrlProxy(new ZmqQueryCtrl(this, proxy->endpoint(), proxy->identity(), ctrl_category));
    }
...
MWQueryProxy ZmqMiddleware::add_query_object(QueryObjectBase::SPtr const& query)
{
    ...
    try
    {
        ...
        return ZmqQueryProxy(new ZmqQuery(this, proxy->endpoint(), proxy->identity(), query_category));
    }
    catch (std::exception const& e) // Should never happen unless our implementation is broken
    {
        // TODO: log this
...
MWRegistryProxy ZmqMiddleware::add_registry_object(string const& identity, RegistryObjectBase::SPtr const& registry)
{
    ...
    try
    {
        ...
        return ZmqRegistryProxy(new ZmqRegistry(this, proxy->endpoint(), proxy->identity(), registry_category, twoway_timeout_));
    }
    catch (std::exception const& e) // Should never happen unless our implementation is broken
    {
        // TODO: log this
...
MWReplyProxy ZmqMiddleware::add_reply_object(ReplyObjectBase::SPtr const& reply)
{
    ...
    try
    {
        ...
        return ZmqReplyProxy(new ZmqReply(this, proxy->endpoint(), proxy->identity(), reply_category));
    }
    catch (std::exception const& e) // Should never happen unless our implementation is broken
    {
        // TODO: log this
...
MWScopeProxy ZmqMiddleware::add_scope_object(string const& identity, ScopeObjectBase::SPtr const& scope)
{
    ...
    try
    {
        ...
        return ZmqScopeProxy(new ZmqScope(this, proxy->endpoint(), proxy->identity(), scope_category, twoway_timeout_));
    }
    catch (std::exception const& e) // Should never happen unless our implementation is broken
    {
        // TODO: log this
...
MWStateReceiverProxy ZmqMiddleware::add_state_receiver_object(std::string const& identity, StateReceiverObject::SPtr const& state_receiver)
{
    ...
    try
    {
        ...
        return ZmqStateReceiverProxy(new ZmqStateReceiver(this, proxy->endpoint(), proxy->identity(), state_category));
    }
    catch (std::exception const& e) // Should never happen unless our implementation is broken
    {
        // TODO: log this
