...
SigReceiverObject::SigReceiverObject(core::posix::ChildProcess::DeathObserver& death_observer)
...
SigReceiverObject::~SigReceiverObject()
{
    ...
    for (auto& scope_process : scope_processes_)
    {
        ...
        {
            cerr << "SigReceiverObject::~SigReceiverObject(): " << e.what() << endl;
        }
    }
...
ScopeMetadata SigReceiverObject::get_metadata(std::string const& scope_id) const
{
    lock_guard<decltype(mutex_)> lock(mutex_);
    // If the id is empty, it was sent as empty by the remote client.
    if (scope_id.empty())
    {
        throw unity::InvalidArgumentException("SigReceiverObject::get_metadata(): Cannot search for scope with empty id");
    }
    ...
    throw NotFoundException("SigReceiverObject::get_metadata(): no such scope: ",  scope_id);
}

MetadataMap SigReceiverObject::list() const
{
    lock_guard<decltype(mutex_)> lock(mutex_);
...
ScopeProxy SigReceiverObject::locate(std::string const& scope_id)
{
...
bool SigReceiverObject::add_local_scope(std::string const& scope_id, ScopeMetadata const& metadata,
                                     ScopeExecData const& exec_data)
{
    ...
    if (scope_id.empty())
    {
        throw unity::InvalidArgumentException("SigReceiverObject::add_local_scope(): Cannot add scope with empty id");
    }
    if (scope_id.find('/') != std::string::npos)
    {
        throw unity::InvalidArgumentException("SigReceiverObject::add_local_scope(): Cannot create a scope with '/' in its id");
    }
...
bool SigReceiverObject::remove_local_scope(std::string const& scope_id)
{
    lock_guard<decltype(mutex_)> lock(mutex_);
    // If the id is empty, it was sent as empty by the remote client.
    if (scope_id.empty())
    {
        throw unity::InvalidArgumentException("SigReceiverObject::remove_local_scope(): Cannot remove scope "
                                              "with empty id");
    }
...
void SigReceiverObject::set_remote_registry(MWRegistryProxy const& remote_registry)
{
    lock_guard<decltype(mutex_)> lock(mutex_);
    remote_registry_ = remote_registry;
}

bool SigReceiverObject::is_scope_running( std::string const& scope_id )
{
    ...
    throw NotFoundException("SigReceiverObject::is_scope_process_running(): no such scope: ",  scope_id);
}

void SigReceiverObject::on_process_death(core::posix::Process const& process)
{
...
SigReceiverObject::ScopeProcess::ScopeProcess(ScopeExecData exec_data)
    : exec_data_(exec_data)
{
}

SigReceiverObject::ScopeProcess::ScopeProcess(ScopeProcess const& other)
    : exec_data_(other.exec_data_)
{
}

SigReceiverObject::ScopeProcess::~ScopeProcess()
{
    try
    {
        kill();
    ...
    {
        cerr << "SigReceiverObject::ScopeProcess::~ScopeProcess(): " << e.what() << endl;
    }
}

SigReceiverObject::ScopeProcess::ProcessState SigReceiverObject::ScopeProcess::state() const
{
    std::lock_guard<std::mutex> lock(process_mutex_);
    return state_;
}

bool SigReceiverObject::ScopeProcess::wait_for_state(ProcessState state, int timeout_ms) const
{
    std::unique_lock<std::mutex> lock(process_mutex_);
    return wait_for_state(lock, state, timeout_ms);
}

void SigReceiverObject::ScopeProcess::exec(core::posix::ChildProcess::DeathObserver& death_observer)
{
    std::unique_lock<std::mutex> lock(process_mutex_);
    ...
    else if (state_ == ScopeProcess::Stopping)
    {
        ...
        {
            cerr << "SigReceiverObject::ScopeProcess::exec(): Force killing process. Scope: \""
            ...
            {
                cerr << "SigReceiverObject::ScopeProcess::exec(): " << e.what() << endl;
            }
        }
    }
...

    {
        ...
        if (process_.pid() <= 0)
        {
            ...
            throw unity::ResourceException("SigReceiverObject::ScopeProcess::exec(): Failed to exec scope via command: \""
    ...
    if (!wait_for_state(lock, ScopeProcess::Running, c_process_wait_timeout))
    {
        ...
        throw unity::ResourceException("SigReceiverObject::ScopeProcess::exec(): exec aborted. Scope: \""
                                       + exec_data_.scope_id + "\" took too long to start.");
    }

    cout << "SigReceiverObject::ScopeProcess::exec(): Process for scope: \"" << exec_data_.scope_id << "\" started" << endl;

    // 4. add the scope process to the death observer
    death_observer.add(process_);
}

void SigReceiverObject::ScopeProcess::kill()
{
    std::unique_lock<std::mutex> lock(process_mutex_);
    kill(lock);
}

bool SigReceiverObject::ScopeProcess::on_process_death(pid_t pid)
{
    std::lock_guard<std::mutex> lock(process_mutex_);
    ...
    if (pid == process_.pid())
    {
        cout << "SigReceiverObject::ScopeProcess::on_process_death(): Process for scope: \"" << exec_data_.scope_id
             << "\" terminated" << endl;
        clear_handle_unlocked();
        return true;
    }
...
void SigReceiverObject::ScopeProcess::clear_handle_unlocked()
{
    process_ = core::posix::ChildProcess::invalid();
    update_state_unlocked(Stopped);
}

void SigReceiverObject::ScopeProcess::update_state_unlocked(ProcessState state)
{
    state_ = state;
    state_change_cond_.notify_all();
}

bool SigReceiverObject::ScopeProcess::wait_for_state(std::unique_lock<std::mutex>& lock,
...
void SigReceiverObject::ScopeProcess::kill(std::unique_lock<std::mutex>& lock)
{
    if (state_ == Stopped)
    {
        return;
    ...
    try
    {
        ...
        if (!wait_for_state(lock, ScopeProcess::Stopped, c_process_wait_timeout))
        {
            ...
            cerr << "SigReceiverObject::ScopeProcess::kill(): Scope: \"" << exec_data_.scope_id
    ...
    {
        cerr << "SigReceiverObject::ScopeProcess::kill(): Failed to kill scope: \""
