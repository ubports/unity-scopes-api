...
MWQueryCtrlProxy SSScopeObject::activate(Result const &result,
  ...
  try {
    ...
    if (!act_base) {
      // TODO: log error, scope returned null pointer.
      throw ResourceException("Scope \"" + runtime_->scope_name() +
                              "\" returned nullptr from activate(\"" +
                              result.uri() + "\")");
    }
  }
  catch (...) {
  ...
  try {
    ...
    // when the query completes, it can tell the ctrl object
    // to destroy itself.
    ActivationQueryObject::SPtr qo(
        make_shared<ActivationQueryObject>(act_base, reply, ctrl_proxy));
    MWQueryProxy query_proxy = mw_base->add_query_object(qo);
...
MWQueryCtrlProxy SSScopeObject::preview(Result const &result,
  ...
  try {
    ...
    QueryObject::SPtr qo(
        make_shared<PreviewQueryObject>(preview_query, reply, ctrl_proxy));
    MWQueryProxy query_proxy = mw_base->add_query_object(qo);
