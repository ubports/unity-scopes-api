...
{
    {
        ...
        EXPECT_EQ("Less than %d", filter1->label_template());
    ...
    {
        EXPECT_THROW(ValueSliderFilter::create("f1", "Less than %d", 10, 1), unity::LogicException);
        EXPECT_THROW(ValueSliderFilter::create("f1", "Less than %d", -1, 10), unity::LogicException);
    }
}
...
{
    {
        FilterState fstate;
        auto filter1 = ValueSliderFilter::create("f1", "Less than %d", 1, 100);
        EXPECT_EQ("Less than 100", filter1->label(fstate));
