...
{
    QActionMetadata* metadata = new QActionMetadata("en", "phone");

    QVariant data(1999);

    // scope data tests
    metadata->set_scope_data(data);

    QVariant stored_data = metadata->scope_data();
    bool ok;
    int stored_number;
    stored_number = stored_data.toInt(&ok);
    EXPECT_EQ(ok, true);
    EXPECT_EQ(stored_number, 1999);
    ...
    QActionMetadata metadata2 = *metadata;
    delete metadata;
